...
export class DGTSourceRemoteService extends DGTSourceService {
    ...
    get(id: string): Observable<DGTSource<any>> {
        this.logger.debug(DGTSourceRemoteService.name, 'Starting to get', { id });
        ...
        return of({ id })
            .pipe(
                map(data => ({ ...data, uri: `${this.config.get(c => c.server.uri)}source/${data.id}` })),
