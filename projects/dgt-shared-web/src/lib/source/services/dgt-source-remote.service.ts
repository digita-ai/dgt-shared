...
export class DGTSourceRemoteService extends DGTSourceService {
    ...
    public get(uri: string): Observable<DGTSource<any>> {
        ...
        return of({ uri }).pipe(
            map((data) => ({
                ...data,
                uri: `${this.config.get((c) => c.server.uri)}source/${encodeURIComponent(data.uri)}`,
            })),
            switchMap((data) =>
                this.store
            ...
            switchMap((data) =>
                this.http.get<DGTSource<any>>(data.uri, { Authorization: `Bearer ${data.accessToken}` }),
            ),
    ...
    public query(filter?: DGTLDFilter): Observable<DGTSource<any>[]> {
        ...
        return of({ filter }).pipe(
            ...
            switchMap((data) =>
                this.http
                    .get<DGTSource<any>[]>(data.uri, { Authorization: `Bearer ${data.accessToken}` })
                    .pipe(map((response) => ({ ...data, response }))),
            ),
            switchMap((data) =>
                data.filter
                    ? this.filters.run<DGTSource<any>>(data.filter, data.response.data)
        ...
        return of({ resource }).pipe(
            ...
            switchMap((data) =>
                data.resource.uri
                    ...
                    : this.http.post<DGTSource<any>>(data.uri, resource, {
    ...
    public refresh(source: DGTSource<any>): Observable<DGTLDResource[]> {
        this.logger.debug(DGTSourceRemoteService.name, 'Refreshing resources for holder');
        ...
        return of({ source }).pipe(
            map((data) => ({
                ...data,
                uri: `${this.config.get((c) => c.server.uri)}source/${encodeURIComponent(data.source.uri)}/refresh`,
            })),
            switchMap((data) =>
                this.store
                    .select((state) => state.app.accessToken)
