...
export class DGTConnectionMockService extends DGTConnectionService {
    ...
    public query<T extends DGTConnection<any>>(filter?: DGTLDFilter): Observable<T[]> {
        ...
        return of({ filter, resources: this.resources }).pipe(
            switchMap((data) =>
                data.filter ? this.filters.run<DGTConnection<any>>(data.filter, data.resources) : of(data.resources),
            ),
        );
    }
    ...
    public getConnectionsWithWebId<T extends DGTConnection<any>>(webId: string): Observable<T[]> {
        ...
        return of(
            this.resources.filter((connection) => connection.configuration && connection.configuration.webId === webId),
        );
    }
