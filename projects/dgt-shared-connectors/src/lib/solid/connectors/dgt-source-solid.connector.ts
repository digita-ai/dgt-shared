...
export class DGTConnectorSolid extends DGTConnector<DGTSourceSolidConfiguration, DGTConnectionSolidConfiguration> {
  ...
  private add<T extends DGTLDResource>(resources: T[], transformer: DGTLDTransformer<T>): Observable<T[]> {
    ...
    return of({ resources, transformer })
      .pipe(
        ...
        switchMap(data => forkJoin(Object.keys(data.groupedEntities).map(uri => this.solid.generateToken(uri, data.connection, data.source)
          .pipe(
            map(token => ({
  ...
  public queryOne<T extends DGTLDResource>(uri: string, exchange: DGTExchange, connection: DGTConnectionSolid, source: DGTSourceSolid, transformer: DGTLDTransformer<T>): Observable<T[]> {
    ...
    return of({ connection, source, uri, exchange })
      .pipe(
        switchMap(data => data.source.state === DGTSourceState.PREPARED ? of(data) : this.solid.prepare(data.source).pipe(
          switchMap(preparedSource => this.sources.save([preparedSource])),
          map(preparedSources => ({ ...data, source: _.head(preparedSources) })),
        )),
        switchMap(data => this.solid.generateToken(data.uri, data.connection, data.source)
          .pipe(map(token => ({
        ...
        switchMap(data => transformer.toDomain([data.resource])
          .pipe(map(resources => ({ ...data, resources: resources.map(resource => ({ ...resource, uri: this.uris.generate(data.resource, 'data') })) })))),
        tap(data => this.logger.debug(DGTConnectorSolid.name, 'Transformed resources', { data })),
  ...
  public updateDiscovery(connection: DGTConnectionSolid, source: DGTSourceSolid, exchange: DGTExchange): Observable<DGTConnectionSolid> {
    this.logger.debug(DGTConnectorSolid.name, 'Starting to update discovery index.', { connection, source, exchange });

    return of({ connection, source, exchange })
      .pipe(
        switchMap(data => this.loadTypeIndexes(data.connection, data.source, data.exchange)
          .pipe(map(typeIndexes => ({ ...data, connection: { ...data.connection, configuration: { ...data.connection.configuration, typeIndexes } } })))),
        switchMap(data => this.loadTypeRegistrations(data.connection, data.source, data.exchange)
          .pipe(map(typeRegistrations => ({ ...data, connection: { ...data.connection, configuration: { ...data.connection.configuration, typeRegistrations } } })))),
        map(data => data.connection),
      )
  }

  public loadTypeIndexes(connection: DGTConnectionSolid, source: DGTSourceSolid, exchange: DGTExchange): Observable<string[]> {
    this.logger.debug(DGTConnectorSolid.name, 'Starting to retrieve all type indexes.', { connection, source, exchange });
    ...
    return of({ connection, source, exchange })
      .pipe(
        switchMap(data => this.queryOne<DGTProfile>(connection.configuration.webId, data.exchange, data.connection, data.source, this.profileTransformer)
          .pipe(map(profiles => ({ ...data, profiles })))),
        map(data => _.flatten(data.profiles.map(profile => [profile.privateTypeIndex, profile.publicTypeIndex]))),
  ...
  public loadTypeRegistrations(connection: DGTConnectionSolid, source: DGTSourceSolid, exchange: DGTExchange): Observable<DGTLDTypeRegistration[]> {
    this.logger.debug(DGTConnectorSolid.name, 'Starting to retrieve all type registrations.', { typeIndexes: connection?.configuration?.typeIndexes, connection });
      ...
      .pipe(
        switchMap(data => forkJoin(
          data.connection.configuration.typeIndexes
            ...
            .map(typeIndex =>
              this.queryOne(typeIndex, data.exchange, data.connection, data.source, this.typeRegistrationsTransformer)
                .pipe(
                  catchError(error => {
                    this.logger.error(DGTConnectorSolid.name, 'Error while querying type registrations', error);
